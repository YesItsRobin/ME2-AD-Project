...
public class ReadStrain extends BaseReader {
    ...
    // can probably be deleted because another we have compacted data
    ...
    //same for this on can be deleted
    public static int findIndex(int group, int number, LocalDateTime begin) throws IOException {
        ...
        while (!done) {
            ...
            if (strain.getDateTime().compareTo(begin) < 0) {
                index += 100000;
            } else if (strain.getDateTime().compareTo(begin) >= 0) {
                index -= 100000;
                done = true;
            }
        }
